...
#define MCONTROL_HIT        (1 << 22)
#define MCONTROL_SELECT     (1 << 21)
#define MCONTROL_TIMING     (1 << 20)
#define MCONTROL_ACTION     (0xf << 12)
// #define MCONTROL_SELECT     (1 << 19)  //older mcontrol
...
#define VMIDMAX      14
#define HGATP64_VMID (((1ULL << VMIDMAX) - 1) << HGATP64_VMID_OFFSET)
...
/* PBMT field in Sv39, Sv48, and Sv57 PTEs from Svpbmt*/
...
// S/VS time compare CSRs (Added by Sstc Extension)
...
#define SUPPORTED_EXCEPTIONS ((1 << CAUSE_FETCH_ACCESS)           | (1 << CAUSE_ILLEGAL_INSTRUCTION)  | (1 << CAUSE_BREAKPOINT)            | \
...
#define SUPPORTED_EXCEPTIONS_HYPERVISOR_DISABLED ((1 << CAUSE_FETCH_ACCESS)           | (1 << CAUSE_ILLEGAL_INSTRUCTION)  | (1 << CAUSE_BREAKPOINT)            | \
                                                  (1 << CAUSE_LOAD_ACCESS)            | (1 << CAUSE_STORE_ACCESS)         | (1 << CAUSE_USER_ECALL)            | \
                                                  (1 << CAUSE_VU_ECALL)               | (1 << CAUSE_SUPERVISOR_ECALL)     | (1 << CAUSE_MACHINE_ECALL)         | \
                                                  (1 << CAUSE_FETCH_PAGE_FAULT)       | (1 << CAUSE_LOAD_PAGE_FAULT)      | (1 << CAUSE_STORE_PAGE_FAULT)      | \
                                                  (1 << CAUSE_INSTRUCTION_CORRUPTION) | (1 << CAUSE_LOAD_DATA_CORRUPTION) | (1 << CAUSE_STORE_DATA_CORRUPTION) | \
...
#define HYPERVISOR_SUPPORTED_EXCEPTIONS ((1 << CAUSE_HS_ECALL) | (1 << CAUSE_VIRTUAL_INSTR_TRAP)    | (1 << CAUSE_FETCH_GUEST_PAGE_FAULT) | \
...
DECLARE_CAUSE("instruction address breakpoint", CAUSE_INSTR_ADDR_BREAKPOINT)
DECLARE_CAUSE("load store amo breakpoint", CAUSE_LOAD_STORE_AMO_BREAKPOINT)
